...
public enum WailaPacketHandler {
    ...
    {
        this.channels = NetworkRegistry.INSTANCE.newChannel("Waila", new ChannelHandler[] { new WailaCodec() });
    ...
    {
        FMLEmbeddedChannel channel = (FMLEmbeddedChannel)this.channels.get(Side.SERVER);
        String codec = channel.findChannelHandlerNameForType(WailaCodec.class);
    ...
    public void writeNBT(ByteBuf target, NBTTagCompound tag)
            throws IOException
    {
        if (tag == null)
        {
    ...
    public NBTTagCompound readNBT(ByteBuf dat)
            throws IOException
    {
    ...
    public void writeString(ByteBuf buffer, String data)
            throws IOException
    {
